...
metadata:
  tags: [ascii, video, terminal, animation]
  tier: task-specific
  domain: creative
...
|------|-------|--------|-----------|
| **Video-to-ASCII** | Video file | ASCII recreation of source footage, with optional motion extrapolation to extend duration without hard loops | `references/inputs.md` § Video Sampling, `references/ascii-domain-extrapolation.md` |
| **Video-to-ASCII (extended)** | Short video + duration target | Motion-compensated interpolation + organic extrapolation in ASCII grid space | `references/ascii-domain-extrapolation.md` |
...
### Extending Short Video Clips — Use ASCII-Domain Extrapolation

When a user wants a 5s clip extended to 60s, **do not default to ffmpeg concat + crossfade**.
That produces hard loops the brain immediately detects. Instead, use **ASCII-domain motion
extrapolation** (`references/ascii-domain-extrapolation.md`):

1. Pre-render source frames to ASCII grids (chars + colors + lum)
2. Compute motion vectors via phase correlation on luminance grids — fast, no full-res warping
3. Interpolate: motion-compensated crossfade between source pairs
4. Extrapolate: extend trajectories with damped velocity + curl noise + breathing pulse
5. Render from modified grids back to pixel canvas

This runs at **10–15 fps on CPU** (vs ~1–2 fps for full-res optical flow). The tradeoff:
- No new visual objects (icon drifts, doesn't sprout wings)
- Global translation only (no local deformation)
- Graceful fade-to-static after ~120+ extrapolated frames

Always present this honestly: "I can extend the motion organically in ASCII space,
but for genuinely new frames beyond the source you'd need Runway/Pika."

### Decision-Making & Output Conventions

When the user asks for a concrete action ("extend this," "make it longer," "loop it"),
**do not present a numbered menu of options**. Pick the best feasible approach from
this skill's toolbox, execute it, and state the honest boundary:

- Bad: "What do you want? 1) Simple loop 2) Ping-pong 3) Slow-mo 4) Extrapolation?"
- Good: "I can extend this to 60s via ASCII-domain motion extrapolation (organic drift,
  no hard loops). I cannot generate genuinely new motion frames without Runway/Pika.
  Shipping now."

If the first result is rejected ("too slow," "too fast," "didn't like it"), do not
rebuild from scratch. Patch the 3-5 tunable parameters and re-render.

For output paths: write video artifacts to `claude-projects/generated/` per vault
taxonomy v3. Never default to Desktop.
...
| File | Purpose |
|------|---------|
| `templates/generative_production_system.py` | Complete working scaffold for generative ASCII videos: 6-scene pipeline, value fields, character palettes, FFmpeg encoding. Copy and modify for new projects. |
| `templates/ascii-domain-extrapolation.py` | Starter script for extending short video clips via motion-compensated interpolation + organic drift in ASCII grid space. Modify INPUT, OUTPUT, TARGET_SEC, then run. |
