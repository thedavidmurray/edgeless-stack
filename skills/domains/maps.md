...
metadata:
  tags: [maps, geocoding, osm, routing]
  tier: task-specific
  domain: product
when_to_apply: >
...
- **API timeouts**: Geocoding calls may timeout (>30s) during peak hours or with complex queries. For demo/build scenarios, use known coordinates from Wikipedia/Wikidata as fallback rather than blocking on API recovery. See `references/maplibre-react-pattern.md` for building visualizations with fallback coordinate strategies.
...
- `references/maplibre-react-pattern.md` — MapLibre GL + React visualization templates
- `references/institutional-data-enrichment.md` — Enriching maps with museum collection data (NGA, Met, etc.)
