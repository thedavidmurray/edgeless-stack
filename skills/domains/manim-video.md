...
**Isolated venv required on macOS**: System-wide `pip install manim` often fails with `pbr` / `pkg_resources` metadata-generation errors. Use a fresh virtualenv:
```bash
...
```
Then run manim via the venv binary: `/tmp/manim-venv/bin/manim -ql script.py SceneName`

**LaTeX minimal install**: Full `mactex` is ~4GB. For text-only animations (no equations), Manim works without LaTeX by using `Text()` instead of `MathTex()`. If equations are needed, install `basictex` + `dvisvgm` rather than full `mactex`.

See `references/setup-pitfalls.md` for full reproduction recipes.
...
**VGroup vs Group — applies to EVERY collection**: When your collection contains ANY `Text()` object, use `Group()` not `VGroup()`. `Text` is an `Mobject` not a `VMobject`. If you hit `TypeError: Only values of type VMobject can be added as submobjects of VGroup`, replace `VGroup()` with `Group()` everywhere in that scene.

**Import `random` explicitly**: `random.uniform()` and `random.randint()` are common for generative layouts. Manim does not auto-import `random`. Always add `import random` at the top of your script alongside `import math`.

**Backstroke for text readability**: When text overlaps complex backgrounds (grids, diagrams, other text), add a dark stroke behind it so the text remains readable without a full background rectangle:
```python
def make_backstroke(mob, width=4):
    mob.set_stroke(BG, width=width, background=True)
    return mob

# Usage: make_backstroke(title) before adding to scene
```
This is how 3Blue1Brown keeps labels readable over busy backgrounds. See `templates/script-starter.py` for a complete working boilerplate that includes all of these patterns.
...
| `references/setup-pitfalls.md` | macOS venv isolation, LaTeX minimal install, pkg_resources workaround |
| `templates/script-starter.py` | Working boilerplate with Group/VGroup fix, backstroke, color constants |
