...
description: >
  ...
  Boy, PICO-8, C64, etc.), and animate them into short videos. Presets cover
  arcade, SNES, and 10+ era-correct looks. Use `clarify` to let the user pick a
  style before generating.
metadata:
  tags: [pixel-art, retro, palette, image]
  tier: task-specific
  domain: creative
when_to_apply: >
...
**Quick demo pattern:**
```bash
# From fc-pro-collection backgrounds (601×600 PNGs)
cd ~/.hermes/skills/creative/pixel-art/scripts
python pixel_art.py "farcaster_sun#10.png" /tmp/out.png --preset nes
# Crop to even dims before video (arcade/apple2/teletext make odd widths)
magick /tmp/out.png -crop 600x600+0+0 +repage /tmp/out.png
python pixel_art_video.py /tmp/out.png /tmp/out.mp4 --scene night --duration 6 --gif
```
...
- **Video encoding requires even dimensions** — libx264 fails with "width/height
  not divisible by 2" on odd-dimension images (e.g., 601x600). This commonly
  happens when source width isn't divisible by the preset's block size:
  - `arcade` (8px): 601px width → 601/8 = 75.125 → upscales to 601px (odd)
  - `apple2` (10px): 601px width → 601/10 = 60.1 → upscales to 601px (odd)
  - `teletext` (10px): same issue
  **Fix**: Crop to even dimensions before animating:
  `magick input.png -crop 600x600+0+0 +repage output.png`
  Or use source images with dimensions divisible by the block size (8, 10, etc.).

## Animation Density Modes

Default particle counts are conservative (~10-40 particles per effect) and may
feel "boring" on larger canvases or for users wanting high-energy motion.

**High-Energy Mode** (patch `pixel_art_video.py`):

| Effect | Default | Amped | Speed Boost |
|--------|---------|-------|-------------|
| Stars | 15 | **45** | — |
| Fireflies | 10 | **35** | 2x faster |
| Leaves | 12 | **40** | 1.7x faster |
| Dust motes | 20 | **60** | 2x faster |
| Sparkles | 10 | **35** | — |
| Rain | 30 | **100** | 2x faster |
| Lightning | 4% chance | **8%** | 2x frequency |
| Bubbles | 15 | **50** | 1.5x faster |
| Embers | 18 | **60** | 1.5x faster |
| Snowflakes | 40 | **120** | 1.5x faster |
| Neon pulse | 8 | **24** | — |
| Heat shimmer | 12 | **36** | — |

**When to use amped mode:**
- User says GIFs are "boring" or lack motion
- Creating hero/social content where animation is the main feature
- Source images are large (600px+) where default particles get lost
- Storm/snow/fire scenes where intensity matters

**Trade-offs:**
- File size increases ~2-3x (more pixel changes per frame)
- Slightly slower render (more particles to composite)
- May look chaotic on very small source images (<200px)
